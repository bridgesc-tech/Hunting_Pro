...
class UpdateManager {
    ...
    setupUI() {
        ...
        if (header) {
            ...
            updateBtn.id = 'checkUpdateBtn';
            updateBtn.className = 'update-check-btn';
            updateBtn.innerHTML = '🔄 Check for Updates';
            updateBtn.title = 'Check for app updates';
            updateBtn.style.cssText = 'position: absolute; top: 10px; right: 10px; padding: 6px 12px; font-size: 12px; background: var(--secondary-color); color: white; border: 1px solid var(--border-color); border-radius: 6px; cursor: pointer;';
            header.style.position = 'relative';
            header.appendChild(updateBtn);
            
            updateBtn.addEventListener('click', () => this.checkForUpdate());
        }
    ...
    showUpdateBanner() {
        ...
        const checkBtn = document.getElementById('checkUpdateBtn');
        if (checkBtn) {
            checkBtn.innerHTML = '🔄 Update Available';
            checkBtn.disabled = false;
        }
    }
    ...
    async applyUpdate() {
        if (!this.registration || !this.registration.waiting) {
            // If no waiting worker, reload to get the latest
