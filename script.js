class ColorDetector {
    ...
    vibrateOnDetection() {
        ...
        if ('vibrate' in navigator) {
            try {
                // Vibrate pattern: short vibration, pause, short vibration
                // This creates a distinctive "double tap" pattern
                navigator.vibrate([100, 50, 100]);
            } catch (error) {
                // Silently fail if vibration doesn't work (some browsers/devices)
    ...
    detectColors() {
        ...
        for (let i = 0; i < data.length; i += 4) {
            ...
            if (isMatch) {
                matchingPixels++;
                mask[y * width + x] = 1;
                // Keep original color for detected areas - outline will be drawn separately
            } else {
...
class MoonService {
    ...
    drawContours(mask, width, height) {
        // Enhanced bright neon green outline for detected areas - more visible and prominent
        this.ctx.strokeStyle = '#00ff00'; // Bright neon green
        this.ctx.lineWidth = 3; // Thicker line for better visibility
        this.ctx.shadowColor = '#00ff00'; // Neon green glow
        this.ctx.shadowBlur = 8; // Increased glow effect
        ...
        // Draw prominent red outlines for each detected region
