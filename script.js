class ColorDetector {
    constructor() {
        ...
        this.hueTolerance = 30; // Wider tolerance for blood (can be bright red to dark brown-red)
        this.saturationMin = 0.2; // Lower saturation - blood can be less saturated, especially when dried
        this.brightnessMin = 0.15; // Lower brightness - blood can be very dark
    ...
    setupEventListeners() {
        ...
        document.getElementById('flashlightToggle').addEventListener('click', () => this.toggleFlashlight());
    ...
    detectColors() {
        ...
        for (let i = 0; i < data.length; i += 4) {
            ...
            if (isRedTarget) {
                ...
                // For blood: allow wider hue range (red to brown-red), lower saturation, and darker values
                // Blood can range from bright red (hue ~0-20) to dark brown-red (hue ~0-30, low brightness)
                ...
                // Special case: very dark pixels might still be blood if they have some red hue
                const isDarkBlood = hsv.v < 0.3 && minDiff <= 40 && hsv.s >= 0.15;
                
                isMatch = (hueMatch && satMatch && brightMatch) || isDarkBlood;
            } else {
            ...
            if (isMatch) {
                matchingPixels++;
                mask[y * width + x] = 1;
                // Keep original color for matching areas
            } else {
        ...
        // Draw improved outline around detected areas
        this.drawContours(finalMask, width, height);
        
        const matchPercentage = matchingPixels / totalSampled;
        const isDetected = matchPercentage > 0; // Any detection counts
...
class WeatherService {
    ...
    async fetchWeatherData(lat, lon) {
        ...
        const response = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,weather_code,surface_pressure&hourly=temperature_2m,weather_code,wind_speed_10m,wind_direction_10m,surface_pressure&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,surface_pressure_max,surface_pressure_min&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America/New_York`
        );
        const data = await response.json();
            ...
            hourly.forEach((hour) => {
                ...
                html += `
                    <div class="hourly-item">
                        ...
                        ${hour.pressure ? `<div class="hourly-pressure">${formatPressure(hour.pressure)}</div>` : ''}
                    </div>
                `;
            });
...
class MoonService {
    ...
    async fetchWeatherData(lat, lon) {
        const response = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m,weather_code&hourly=temperature_2m,weather_code,wind_speed_10m,wind_direction_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America/New_York`
        );
        const data = await response.json();
        return data;
    }
    ...
    drawContours(mask, width, height) {
        // Use brighter, more visible color for blood spots
        this.ctx.strokeStyle = '#ff0000';
        this.ctx.lineWidth = 2; // Thinner line for small spots
        this.ctx.shadowColor = '#ff0000';
        this.ctx.shadowBlur = 6;
        this.ctx.fillStyle = 'rgba(255, 0, 0, 0.2)'; // Light fill for visibility
        
        // Improved contour detection - find connected regions
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                ...
                if (mask[idx] === 1 && visited[idx] === 0) {
                    ...
                    const region = this.traceRegion(mask, visited, x, y, width, height);
                    // Lower threshold for blood spots - detect even very small spots (3+ pixels)
                    if (region.length > 3) {
                        regions.push(region);
                    }
                }
        ...
        // Draw outlines and fills for each region (better visibility for small blood spots)
        regions.forEach(region => {
            if (region.length > 0) {
                this.ctx.beginPath();
                this.ctx.moveTo(region[0].x, region[0].y);
                for (let i = 1; i < region.length; i++) {
                    this.ctx.lineTo(region[i].x, region[i].y);
                }
                this.ctx.closePath();
                // Fill small regions for better visibility
                if (region.length < 50) {
                    this.ctx.fill();
                }
                this.ctx.stroke();
    ...
    traceRegion(mask, visited, startX, startY, width, height) {
        const region = [];
    ...
    displayMoonData(weatherData, location) {
        ...
        for (let i = 0; i < Math.min(7, daily.time.length); i++) {
            ...
            if (dayDate <= today) continue; // Skip today and past days
...
class MapTrackingService {
    ...
    initMap() {
        ...
        // Initialize map with satellite view using free Esri World Imagery
        this.map = L.map('map', {
            zoomControl: true,
        ...
        // Set a default view first (center of USA)
        this.map.setView([39.8283, -98.5795], 13);
    ...
    startTracking() {
        ...
        // Watch position updates with more lenient settings
        this.watchId = navigator.geolocation.watchPosition(
            ...
            {
                enableHighAccuracy: false, // Use less accurate but more reliable
    ...
    updatePosition(position) {
        ...
        this.trackingPath.push([lat, lon]);
        ...
        if (this.pathPolyline) {
            this.pathPolyline.setLatLngs(this.trackingPath);
        } else {
            this.pathPolyline = L.polyline(this.trackingPath, {
                color: '#ff0000',
                weight: 4,
                opacity: 0.8
            }).addTo(this.map);
...
class StandsService {
    ...
    saveLocations() {
        localStorage.setItem('huntingStands', JSON.stringify(this.locations));
    }
    ...
    setupEventListeners() {
        ...
        document.getElementById('saveStandBtn').addEventListener('click', () => this.saveStand());
    ...
    viewLocationMap(locationIndex) {
        ...
        if (allIcons.length > 0) {
            ...
            // No icons - set default view
            this.locationMap.setView([39.8283, -98.5795], 13);
        }
    ...
    async shareLocationToFirebase(location) {
        if (!this.sharingService.database) {
            console.warn('Firebase not configured. Location will be saved locally only.');
            return;
        }
    ...
    openStandModal(editIndex = null) {
        ...
        if (!this.standMapPicker) {
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    ...
                    () => {
                        this.standMapPicker.setView([39.8283, -98.5795], 13);
                    }
                );
            } else {
                this.standMapPicker.setView([39.8283, -98.5795], 13);
        ...
        if (editIndex !== null) {
            ...
            // New stand - center on user or default
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (pos) => {
                        this.standMapPicker.setView([pos.coords.latitude, pos.coords.longitude], 15);
    ...
    setupDragAndDrop() {
        ...
        // Set up drag start for icon buttons
        draggableIcons.forEach(icon => {
            icon.addEventListener('dragstart', (e) => {
                draggedIconType = icon.dataset.iconType;
        ...
        // Prevent default drag behavior on map
        this.standMapPicker.getContainer().addEventListener('dragover', (e) => {
            e.preventDefault();
            e.dataTransfer.dropEffect = 'copy';
        });
        
        // Handle drop on map
        this.standMapPicker.getContainer().addEventListener('drop', (e) => {
            e.preventDefault();
            
            if (!draggedIconType) return;
            
            // Get the drop position on the map
            const mapContainer = this.standMapPicker.getContainer();
            const rect = mapContainer.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
            const point = L.point(x, y);
            const latlng = this.standMapPicker.containerPointToLatLng(point);
            
            // Place the icon at the drop location
            this.placeIconOnMap(latlng.lat, latlng.lng, draggedIconType);
        });
    }
    ...
    saveStand() {
        ...
        // Save current map view if not set
        if (!this.standMapView) {
            const center = this.standMapPicker.getCenter();
            const zoom = this.standMapPicker.getZoom();
            this.standMapView = {
